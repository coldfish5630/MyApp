...
public class MovieController {
    ...
    @GetMapping("/id")
    public ResponseEntity<Optional<Movie>> getSingleMovie(@RequestParam("id") ObjectId id) {
        return new ResponseEntity<Optional<Movie>>(movieService.singleMovieById(id), HttpStatus.OK);
    }

    @GetMapping("/imdbId")
    public ResponseEntity<Optional<Movie>> getSingleMovieByImdbId(@RequestParam("imdbId") String imdbId) {
